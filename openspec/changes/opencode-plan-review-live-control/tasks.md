...
- [x] 13.1 **RED: Write TUI plugin boot integration test**. RESCUED per ADR-9. Assert: (a) default export shape is `{ id: string, tui: TuiPlugin }` — `id` is present and non-empty; (b) TUI plugin boots without error when mocked `TuiPluginApi` is passed; (c) `getSharedState()` is called at boot and returns the state set by server plugin; (d) sidebar slot is registered via `api.slots.register`; (e) keybinds deferred to 13.5. 20 tests written. (Files: `packages/opencode/src/__tests__/tui-plugin/index.test.ts`)
- [x] 13.2 **GREEN: Implement `tui-plugin/index.ts`**. RESCUED per ADR-9. TUI plugin entry. Default export: `{ id: "bg-subagents-tui", tui: TuiPlugin }`. `id` REQUIRED — runtime throws `TypeError: Path plugin ... must export id` if missing (see spike finding). Boot sequence: read SharedPluginState (graceful if undefined), register sidebar slot (12.7), wire setInterval heartbeat + lifecycle.onDispose cleanup. Keybinds (Ctrl+B, Ctrl+F) deferred to 13.5. 363 tests passing (+20). (Files: `packages/opencode/src/tui-plugin/index.ts`)
- [x] 13.3 **Add `./tui` subpath export in `packages/opencode/package.json`**. RESCUED per ADR-9. Added to `exports`: `"./tui": { "types": "./dist/tui-plugin/index.d.ts", "import": "./dist/tui-plugin/index.js" }`. `tsconfig.json` already covers `src/**/*.ts` so tui-plugin/ will compile to `dist/tui-plugin/` on next build. `files: ["dist", "src", ...]` covers both. (Files: `packages/opencode/package.json`)
- [x] 13.4 **Verify `./tui` subpath included in npm pack output**. FINDING: `pnpm pack --json` confirms `src/tui-plugin/index.ts` IS included (via `files: ["src"]`). `dist/tui-plugin/index.js` is NOT in pack yet — dist is stale (build not run per hard rule "Never build after changes"). This is expected and NOT a blocker: dist will include tui-plugin/ after the next `pnpm -r run build` (e.g. at release time in Phase 17). tsconfig.json + files array are both correctly configured. (Files: verification step — blocker: requires build before dist/tui-plugin/ appears in pack)
...
- [x] 15.1 **Update `packages/opencode/README.md`**. Full v1.0 rewrite: server features (PolicyResolver, messages.transform, /task commands), TUI features (sidebar, Ctrl+B/F/↓, plan-review dialog, tui.json wiring), configuration reference, troubleshooting, architecture overview. Migration note pointing to `docs/migration-v0.1-to-v1.0.md`. (Files: `packages/opencode/README.md`)
- [x] 15.2 **Update `docs/skills/bg-subagents/SKILL.md`**. v1.0 content update: PolicyResolver overview, two-step install (opencode.json + optional tui.json), configuration reference (bgSubagents.policy key), /task command reference including /task policy and /task move-bg, TUI keybinds table, observability paths. (Files: `docs/skills/bg-subagents/SKILL.md`)
- [x] 15.3 **Update `docs/architecture.md`**. Full v1.0 rewrite: ASCII component diagram (server plugin + TUI plugin + SharedPluginState bridge + core), data flow and UI flow sections, component responsibilities table, hook wiring tables (v14, legacy, TUI), SharedPluginState bridge docs, two-config-file install section. (Files: `docs/architecture.md`)
- [x] 15.4 **Create `docs/migration-v0.1-to-v1.0.md`**. User-facing migration guide: 6 breaking changes (config path, plugin key, no picker, policy location, OpenCode version, log path), upgrade steps, new features overview, rollback instructions, getting help. (Files: `docs/migration-v0.1-to-v1.0.md`)
- [x] 15.5 **Update `docs/upstream/gentle-ai-pr.md`**. Refreshed to v1.0 actual scope: TUI layer (sidebar, keybinds, dialog), `./tui` subpath export, two-config-file install, migration guide link, demo GIF placeholders (Phase 16), issue #373 gate reference, @Maicololiveras handle throughout. (Files: `docs/upstream/gentle-ai-pr.md`)
- [x] 15.6 **Update root `README.md`**. Added v1.0-ready badge, two-config-file install section (opencode.json + tui.json), link to packages/opencode/README.md, link to migration guide, Documentation table with all key docs, npm package @maicolextic/bg-subagents-opencode v1.0 status. (Files: `README.md`)
- [x] 15.7 **Update `CHANGELOG.md` via changeset**. Created `.changeset/release-v1-0-0.md` with major bump for opencode, minor for core, patch for protocol. Summary: PolicyResolver, /task commands, TUI layer, ./tui subpath export, zero stdout, 788 tests green. (Files: `.changeset/release-v1-0-0.md`)
- [x] 15.8 **Fix SKILL.md alignment with gentle-ai conventions**. Added YAML front-matter (name, description, license, metadata.author, metadata.version) per gentle-ai skill format. Corrected all `~/.config/opencode/config.json` references to `~/.config/opencode/opencode.json`. Added TUI install section documenting `tui.json` step. (Files: `docs/skills/bg-subagents/SKILL.md`)
- [x] 15.9 **Draft OpenCode docs PR for `tui.json` documentation**. Created `docs/upstream/opencode-docs-pr-draft.mdx` — paste-ready .mdx content for `packages/web/src/content/docs/plugins.mdx` in anomalyco/opencode. Covers: two-config-file architecture, `id` REQUIRED gotcha, minimal 20-line example, TuiPluginApi surface table, install options (project-local / global / npm ./tui subpath), troubleshooting section. (Files: `docs/upstream/opencode-docs-pr-draft.mdx` NEW)
